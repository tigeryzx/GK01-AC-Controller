...
```bash
# 擦除
python -m esptool --port COM5 --baud 115200 --before no-reset erase-flash

# 刷写（推荐 combined.bin，一条命令刷完）
python -m esptool --port COM5 --baud 115200 --before no-reset write-flash -fm dout 0x0 combined-v3.0.0.bin
```
...
设备进入 STA Home 模式后，支持通过 MQTT 接入云端平台或本地服务器。WebUI「设置 → MQTT」可配置以下参数。

WebUI 提供**连接类型**下拉菜单，支持四种模式：

| 模式 | 说明 |
|------|------|
| 自动 | 根据用户名/密码自动判断（兼容旧版行为） |
| 标准 MQTT / HA | 适用于 Home Assistant、Mosquitto 等需要用户名密码认证的服务器 |
| 巴法云 / 米家 | 适用于巴法云平台，支持米家语音控制 |
| 无认证 | 适用于局域网内开放的 MQTT 服务器 |

#### 方式一：标准 MQTT（Home Assistant 等）

适用于自建 MQTT 服务器或 Home Assistant 的 Mosquitto 等需要用户名密码认证的场景。

| 参数 | 填写 |
|------|------|
| 服务器 | `192.168.x.x`（你的 MQTT 服务器地址） |
| 端口 | `1883` |
| 用户名 | MQTT 账号 |
| 密码 | MQTT 密码 |
| Topic | `ir_ac`（自定义，HA 用此值注册 climate 实体） |

Client ID 自动生成为 `IR-AC-{chipId}`，连接时携带用户名和密码。

#### 方式二：巴法云（Bemfa）/ 米家

巴法云 MQTT 要求私钥作为 Client ID 认证。固件采用巴法云官方文档中的方式一：
...
| 用户名 | **留空**（系统自动隐藏） |
| 密码 | 你的私钥（private key） |
| Topic | `你的主题名005`（必须以 `005` 结尾，控制台中新建的主题） |

用户名为空时，固件将密码字段直接用作 Client ID，不传用户名密码，符合巴法云文档中的方式一。

> **重要**：Topic 必须以 `005` 结尾，否则巴法云无法识别为空调设备。WebUI 会校验主题格式，不符合规则时拒绝提交。

> 注意：巴法云的 UID 和主题名是不同的字段，MQTT 连接用的是**主题名**，不是 UID。

**巴法云消息格式**：

设备订阅主主题 `{topic}` 接收控制指令，消息格式为：

```
on#mode值#温度#风速值#左右扫风#上下扫风
off#mode值#温度#风速值#左右扫风#上下扫风
```

| 字段 | 说明 | 值示例 |
|------|------|--------|
| 开关 | `on` / `off` | `on` |
| 模式 | 1=自动 2=制冷 3=制热 4=送风 5=除湿 6=睡眠 7=节能 | `2`（制冷） |
| 温度 | 16-32 整数 | `26` |
| 风速 | 0=自动 1-5 | `0`（自动） |
| 左右扫风 | 0=关闭 1=开启 | `0` |
| 上下扫风 | 0=关闭 1=开启 | `0` |

示例：`on#2#26#0#0#0` 表示开机、制冷、26°C、风速自动、扫风关闭。

> **米家/Bemfa 注意**：米家是单向控制通道，设备状态不会同步到米家。连接时会自动跳过 retained 消息，避免重启后误执行上次命令。
...
Client ID 自动生成为 `IR-AC-{chipId}`，匿名连接。

#### 认证逻辑总结

```
连接类型 = 自动？
  ├─ 是 → 按用户名/密码自动判断（兼容旧版行为）
  └─ 否 → 按选择的模式连接
```

#### MQTT 订阅的 Topic

设备订阅以下 Topic 接收控制指令（`{topicBase}` 为配置的 Topic 值）：

**标准 MQTT / HA 模式**：

| Topic | 说明 | 值示例 |
|-------|------|--------|
| `{topicBase}/mode/set` | 设置模式 | `cool` / `heat` / `auto` / `dry` / `fan_only` / `off` |
| `{topicBase}/temperature/set` | 设置温度 | `26` |
| `{topicBase}/fan/set` | 设置风速 | `auto` / `1` / `2` / `3` |

设备发布状态到 `{topicBase}/state`、`{topicBase}/mode_state` 等子主题，支持 HA Climate 自动发现。

**巴法云 / 米家模式**：

| Topic | 说明 | 值示例 |
|-------|------|--------|
| `{topicBase}` | 接收控制指令 | `on#2#26#0#0#0` |

设备**不发布状态**到巴法云，避免米家收到状态后触发循环命令。

### Home Assistant 接入指南

以下是在 Docker 环境中从零搭建 MQTT 并接入 HA 的完整流程。

#### 第一步：部署 Mosquitto MQTT Broker

在任意目录创建 `mosquitto/docker-compose.yml`：
...
> 局域网使用可设置 `allow_anonymous true` 免密码。如需认证，改为 `allow_anonymous false` 并添加 `password_file /mosquitto/config/password.txt`，然后执行 `docker compose run --rm mosquitto mosquitto_passwd -c /mosquitto/config/password.txt your_username` 生成密码。

#### 第二步：在 HA 中添加 MQTT 集成

HA 新版本（2023.x+）不再支持 `configuration.yaml` 中的 `broker`/`port` 旧配置格式，需通过 UI 添加：

1. 进入 **设置 → 设备与服务 → 添加集成**
2. 搜索 **MQTT**，选择 MQTT（Mosquitto）
3. 填写连接信息：

   | 参数 | 值 |
   |------|------|
   | Broker | HA 宿主机局域网 IP（如 `192.168.x.x`），若 HA 使用 `network_mode: host` 则填 `localhost` |
   | Port | `1883` |
   | 用户名 | （留空或按需填写） |
   | 密码 | （留空或按需填写） |

4. 添加完成后，进入该 MQTT 集成的 **配置** 页面，确认 **Discovery** 已开启，前缀为 `homeassistant`
5. 重启 HA：`docker restart ha`

> 若 HA 与 Mosquitto 在同一 Docker 网络中，Broker 可填容器名 `mosquitto`；若 HA 使用 `network_mode: host`，直接填 `localhost`。
...
| 参数 | 值 |
|------|------|
| 服务器 | 运行 Mosquitto 的服务器局域网 IP |
| 端口 | `1883` |
| 用户名 | 与 Mosquitto 一致（匿名则留空） |
| 密码 | 与 Mosquitto 一致（匿名则留空） |
| Topic | `ir_ac`（自定义，HA 会以此注册 climate 实体） |

保存后设备重启，TTL 串口显示 `[MQTT] Connected` 即成功。

#### 验证连接

```bash
# 查看 Mosquitto 是否有客户端连接
docker logs mosquitto --tail 20

# 监听所有 MQTT 消息，确认设备在发布数据
...
```
homeassistant/climate/ir-ac-xxxx/config  {"name":"IR AC",...}
ir_ac/availability online
ir_ac/state off
ir_ac/mode_state cool
ir_ac/temperature_state 26
ir_ac/fan_state auto
```

HA「设置 → 设备与服务」中会出现名为 "IR AC" 的空调设备，支持模式/温度/风速控制。若未出现，等几分钟或重启设备让 `publishDiscovery()` 再次发送自动发现消息。
...
| v3.1 | 巴法云/米家 MQTT 集成：WebUI 连接类型下拉菜单（自动/标准MQTT/巴法/匿名）；巴法 AC 消息格式解析（on/off#mode#temp#fan#swing_lr#swing_ud）；Topic 005 格式校验；连接后跳过 retained 消息防误操作；巴法模式跳过状态回写防循环命令 |
