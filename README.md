...
mqtt_user 为空？
  ├─ 是 → mqtt_pass 为空？
  │        ├─ 是 → 匿名连接，clientId = IR-AC-{chipId}
  │        └─ 否 → clientId = mqtt_pass，无用户名密码（巴法云）
  └─ 否 → clientId = IR-AC-{chipId}，携带 mqtt_user / mqtt_pass（HA 等）
```
...
设备发布状态到 `{topicBase}`（JSON 格式），支持 HA Climate 自动发现。
